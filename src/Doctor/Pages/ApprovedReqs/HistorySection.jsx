        ...
        {Userhistory && Userhistory.data && (
          <div>
            {Userhistory.data.map(record => {
              ...
              const medications = record.medications.map(med => med.name).join(', ');
              const diagnoses = record.diagnoses.map(diagnosis => diagnosis.name).join(', ');
              const labTests = record["Lab Tests"].map(test => test.english_name).join(', ');
              ...
              const filterMatch = selectedFilter === "All" || selectedFilter === englishName;
                ...
                filterMatch && (
                  <div key={record.id} className=''>
                    <div className='col-12'>
                      ...
                      <p className='fs-6 text-gray'>Medications: {medications}</p>
  ...
  {files.map((file, index) => (
    <li key={index} className="bg-white border-0 "> 
      <button
        ...
        onClick={() => window.open(record.Files[index], '_blank')}
      >
        Document {index + 1}
      </button>
    </li>
      ...
      <button className='border-0 p-1 active-button rounded-2' onClick={() => setPopup(true)}>Add Medical History +</button>
       ) : null}
      {popup && (
